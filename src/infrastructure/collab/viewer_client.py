...
class ViewerClient:
    ...
    async def connect(self, on_output: Callable[[bytes], None] | None = None) -> None:
        ...
        self._task = asyncio.create_task(self._receive_loop(on_output))

    async def _receive_loop(self, on_output: Callable[[bytes], None] | None) -> None:
        if self._ws is None:
            return
        try:
